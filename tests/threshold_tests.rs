...
fn test_calculate_consensus_result_variants() {
    ...
    assert_eq!(
        calculate_consensus_result(&votes, 3, 2.0 / 3.0, false),
        Some(true)
    );
    ...
    votes.insert(vec![3], no_vote(3));
    assert_eq!(
        calculate_consensus_result(&votes, 3, 2.0 / 3.0, true),
        Some(false)
    );
    ...
    votes.insert(vec![2], no_vote(2));
    assert_eq!(
        calculate_consensus_result(&votes, 2, 2.0 / 3.0, true),
        Some(false)
    );
    assert_eq!(
        calculate_consensus_result(&votes, 2, 2.0 / 3.0, false),
        Some(false)
    );
